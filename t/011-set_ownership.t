...
BEGIN {
    *CORE::GLOBAL::getpwnam = sub {
        $args->[0] = \@_;
        return defined( $data->{getpwnam} )
          ? @{ $data->{getpwnam} }
          : qw(user xyz 123 1234);
    };
    *CORE::GLOBAL::getgrnam = sub {
        $args->[1] = \@_;
        return
          defined( $data->{getgrnam} ) ? @{ $data->{getgrnam} } : qw(group xyz 456);
    };
...
sub test_so {
    ...
    my $fn = 'myfile';
    is( File::AtomicWrite::_set_ownership( $fn, $owner ),
        1, "_set_ownership ok with owner $owner" );
...
test_so( "myuser:mygroup",
    [ ['myuser'], ['mygroup'], [ 123, 456, 'myfile' ] ] );
